...
import { type BottomBarTab, WorkspaceBottomBar } from './AdminBottomBar';
...
function TrimesterTab({
  ...
  const [assignmentScrollRequest, setAssignmentScrollRequest] = useState(0);

  useEffect(() => {
    if (!assignmentScrollRequest || typeof document === 'undefined') return;
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    document.getElementById('trimester-assignment-form')?.scrollIntoView({
      behavior: reduceMotion ? 'auto' : 'smooth',
      block: 'start',
    });
  }, [assignmentScrollRequest]);
  const [sheetSearch, setSheetSearch] = useState('');
  ...
  const editSheetTrimester = (sheet: AcademicSheet) => {
    ...
    setAssignmentScrollRequest((current) => current + 1);
  };
  ...
  return (
    <>
      ...
      <Section nativeID="trimester-assignment-form" title="Asignar trimestre a fichas" subtitle="Selecciona una fecha creada, define el número y elige una o varias fichas">
        <View style={styles.formCard}>
  ...
  const items = Children.toArray(children)
    ...
    .map((child, index) =>
      typeof child === 'string' || typeof child === 'number'
        ? <Text key={index} style={styles.cardText}>{child}</Text>
        : child
    );
...
function Section({ children, subtitle, title, nativeID }: { children: ReactNode; subtitle: string; title: string; nativeID?: string }) {
  ...
  return (
    <View nativeID={nativeID} style={styles.sectionBlock}>
